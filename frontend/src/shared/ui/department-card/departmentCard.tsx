    ...
    return <>
        <div className="department-card">
            <div className="department-card--heading">
                <h3 className="department-card--heading__title">{props.departmentName}</h3>
                <div className="department-card--actions">
                    <div className="department-card--actions__edit" onClick={() => setIsEditModalOpen(true)}>
                    ...
                    <div className="department-card--actions__delete" onClick={() => setIsDeleteModalOpen(true)}>
            ...
            <p className="department-card--info">Директор: {props.departmentDirector}</p>
            <Link className="department-card--link" to={`/application/department/${props.departmentId}`}>Перейти к кафедре</Link>
        </div>
        <AnimatePresence>
            {isEditModalOpen
                && <BaseModal onClose={() => setIsEditModalOpen(false)}>
                    <DepartmentChangeForm departmentId={props.departmentId} departmentName={props.departmentName} departmentDirector={props.departmentDirector} instituteId={props.instituteId}/>
                </BaseModal>
            }
        </AnimatePresence>
        <AnimatePresence>
            {isDeleteModalOpen
                && <BaseModal onClose={() => setIsDeleteModalOpen(false)}>
                    <div className="department-delete">
                        <h3 className="department-delete--title">{props.departmentName}</h3>
                        <div className="department-delete--buttons">
                            <a className="department-delete--link" href="">Удалить кафедру</a>
                        </div>
                    </div>
                </BaseModal>
            }
